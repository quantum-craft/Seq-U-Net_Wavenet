...
parser.add_argument("--levels", type=int, default=10, help="# of levels (default: 10)")
parser.add_argument(
    "--features",
    type=int,
    default=128,
...
parser.add_argument(
    "--log_dir", type=str, default="logs/wavenet", help="Folder to write logs into"
)
...
parser.add_argument(
    "--hourglass", type=int, default=1, help="How many hourglasses for Seq-U-Net"
)
parser.add_argument(
    "--output_size",
    type=int,
...
parser.add_argument(
    "--model", type=str, default="wavenet", help="Model to use (wavenet/sequnet)"
)
...
if args.cuda:
    ...
    if torch.cuda.is_available():
        print(
            "WARNING: You chose to run CPU mode, but have a CUDA device. You probably want to run with --cuda"
        )
    dtype = torch.FloatTensor
    ltype = torch.LongTensor

torch.backends.cudnn.benchmark = (
    True  # This makes dilated conv much faster for CuDNN 7.5
)
...
if args.model == "wavenet":
    ...
    model = Sequnet(
        ...
        NUM_CLASSES,
        kernel_size=5,
        target_output_size=args.output_size,
        depth=args.depth,
    )
else:
...
### DATASET
train_audio_data = AudioData(
    os.path.join(args.preprocessed_dataset_dir, "audio_train.hdf5"),
    sr=args.sr,
    channels=1,
)
...
test_audio_data = AudioData(
    os.path.join(args.preprocessed_dataset_dir, "audio_test.hdf5"),
    sr=args.sr,
    channels=1,
)
...
train_data = AudioDataset(
    ...
    context_front=model.input_size - model.output_size + 1,
    hop_size=40000,
    random_hops=True,
